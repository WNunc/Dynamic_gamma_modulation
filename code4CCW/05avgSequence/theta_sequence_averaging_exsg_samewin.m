...
subfolder = 'Tseq\';
case1 = '-ontrack_exsg'; %exclude sgamma
ncells = 3;nspkikes = 5;
...
% for ns = [3 4 6 7 8 11 13]% 7 sessions
for ns = [1:26,29,31:isession]
    path_ns = path{ns};
    disp(path_ns)
    ...
    for nl = 1:5
        ...
        file_input1 = strcat(path_ns,subfolder,'scores',num2str(nseg),'-',num2str(nl),case1,'.mat');
        load(file_input1)
        ...
        file_input5 = strcat(path_ns,subfolder,'data_theta_seq_info-ontrack_lap',num2str(nl),case2,case3,'_v5.mat');
        load(file_input5)
...
%             thetacycle.cycle_ts{nl}(:,2)<TS_running(nl,2));
        ...
        spikes = spikes(cind_ot_exsg,:);
        cell_ontrack = length(cind_ot_exsg)
...
%         TSS_mean3 = mean(TSS_temp3,3,'omitnan');
       %%
        ...
        saveas(figure(1),[path_ns subfolder 'decoding_single_lap',num2str(nl),case1,'.png'])
        close(figure(1))
        figure(2); %对齐位置
...
%         plot_NPX(NPX3(28:63,:),mid_tbins,cycind,abc)
    ...
    saveas(figure(2),[path_ns,subfolder,'thetaseq_strcture',case1,'1',case2,case3,'.png'])
    saveas(figure(3),[path_ns,subfolder,'thetaseq_strcture',case1,'2',case2,case3,'.png'])

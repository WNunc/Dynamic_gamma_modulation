...
for ns = 1:isession
    path_ns = path{ns};
    cd(path_ns);
    file_output = 'Tseq-CCW\BayesData_CircMap_wn_dt40ms_5ms_1cells_1.mat';
    load (file_output)
    for nt = 1:1 % trial number
        ...
        for nl = 1:laps
            ...
            dir_input = strcat(path{ns},'Tseq-CCW\scores',num2str(nt)','-',num2str(nl),subfix,'_v5.mat');
...
%         end
%         for nl = 1:laps
%             file_output = strcat(path{ns},'Tseq-CCW\scores',num2str(nt),'-',num2str(nl),'-ontrack-wn.mat');
%             load (file_output)
%             Scores = scores;
%             fprintf(['%' num2str(nt) '$s'  '  lap-%4$d \n'],'pre', 'samp', 'test', nl);
%             dir_input = strcat(path{ns},'Tseq-CCW\scores',num2str(nt),'-',num2str(nl),'-wn.mat');
%             file_input = dir_input;
%             load (file_input,'scores')
%             Scores{nt}(nl,10:21) = scores(nl,10:21); 
%             scores = Scores{1};

...
for ns = 1:isession
    ...
    for nt = 1:3
        ...
        for nl = 1:laps
            file_input1=strcat(path_ns,'scores',num2str(nt),'-',num2str(nl),'.mat');
            load(file_input1);
            ...
            for m = 1:length(t_thc)-1
                for n=1:13
                    if pos(m,1)+n-1>=90
                        NXP(n,idxp(m):idxp(m+1))=Pxn(90,idxp(m):idxp(m+1));
                    else
                        NXP(n,idxp(m):idxp(m+1))=Pxn(pos(m,1)+n-1,idxp(m):idxp(m+1));
                    end
                end
            end
            ...
            file_output=strcat(path_ns,'info',num2str(nt),'-',num2str(nl),'.mat');

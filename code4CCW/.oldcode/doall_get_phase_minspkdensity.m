...
for ns = 1:isession
    ...
    [~,indp]= min(phasesmooth(bars+1:2*bars));%!!!!!!!!!!!!!!!!!!!!!!!!!caution!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    max_phase = mod(bincentre(indp),360)
    title(['min spike density phase: ' num2str(max_phase)])
    scatter(bincentre(indp+bars),phasesmooth(indp+bars),'Marker','v','LineWidth',10)
    xticks([0 360 720 1080])
    xticklabels({'0','360', '720', '1080'})
    hold off
    saveas(gcf,[subfolder 'hist_AllLap_thetaphase_',num2str(nbars),'bars-min.png'])
    save([path{ns},subfolder, 'data_AllLap_thetaphase_',num2str(nbars),'bars-min.mat'],...

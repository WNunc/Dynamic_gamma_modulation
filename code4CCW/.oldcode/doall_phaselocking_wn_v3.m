...
% edited by WN on 20220526 calculate phase through EEG in scores 
...
for ns = 1:isession
    ...
    % read timestamps
    Ts_start_stop{1,1} = Ts_prerunning{1,2}./ 1000000;
    ...
    for ii = 1:Ncell
        ...
        CSC0{ii,2} = str2double(TT0{ii}(3:TTind-1));
        [~,EEGind(ii,1)] = ismember(CSC0{ii,2},CSClist_ns);
    end
    nseg = 1;
    ...
    for nc = 1:Ncell
        ...
        if nc == 1 || (nc>=2 && (~strcmp(Current, CSC0{nc,1})))
            ...
            [sigTheta,sigTheta_filtwts]= eegfilt(sample', 2000, 4, 12,length(sample'),[],0,'fir1',0); % pop-up window mode
            [sigGammaS,sigGammaS_filtwts]= eegfilt(sample', 2000, 25, 55,length(sample'),[],0,'fir1',0);
            [sigGammaF,sigGammaF_filtwts]= eegfilt(sample', 2000, 65, 100,length(sample'),[],0,'fir1',0);
            fprintf('\n')
    ...
    file_output = [subfolder 'data_phaselocking_TSlap.mat'];

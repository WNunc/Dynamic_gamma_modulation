function [windows,ind_tet,name_tet,cutph,cutph2] = thetawindows_all_CT_v4_EEG_wn(csclist,TTList,celldir,limitTS)
...
% edited on 07/24/2017
% modified from thetawindows_all_CT_v3_EEG
...
% output: cutph = all session spike
%         cutph2 = ontrack spike
path = pwd;
...
%     spikes{nc,1} = get_spike_times_within_specific_eeg_windows(w(:,1),w(:,2),tempspikes); %to theta filter the spikes
    ...
    spkind = find(tempspikes>=limitTS(1,1)&tempspikes<=limitTS(1,2)...
        |tempspikes>=limitTS(2,1)&tempspikes<=limitTS(2,2)...
        |tempspikes>=limitTS(3,1)&tempspikes<=limitTS(3,2)...
        |tempspikes>=limitTS(4,1)&tempspikes<=limitTS(4,2)...
        |tempspikes>=limitTS(5,1)&tempspikes<=limitTS(5,2));
    spikesot{nc,1} = tempspikes(spkind);
end
% cd ../
...
for i = 1:size(spikes,1)
    ...
    spkotphases = [spkotphases, tphase(SpikeTStoEegIndex(spikesot{i,1},eTS,fs))];
end
...
cutph = xph(ind);
[~,ind] = min(hist(spkotphases,xph));
cutph2 = xph(ind);
temp_theta_ph_spkmin%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!%

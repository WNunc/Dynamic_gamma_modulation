...
[t, x, y] = Nlx2MatVT(file,fieldSelection,extractHeader,extractMode);% get time and coordinate

%tracklength = 193; %track length in cm
% scale = 0.275;% Conversion from pixels to cm, frame is 640 by 480
x = x * scalex; % == transfer to cm ==
y = y * scaley;
t = t ./ 1000000; % transfer timestamp to second
...
% find the segments
ind = find(diff(t)>1); % number of time different
...
for nseg = 1:size(ind_seg,1)
    ...
    %Clear zero values
    ind = find((y0>0 & x0>0)==1 & ~isnan(x0));% so where is the overall origin point?
    
    if ~isempty(ind)
        x0_center = x0-track_center(1);% distance between center and [x0 y0]
        y0_center = y0-track_center(2);
        ang0 = nan(size(x0));
        ind0=find(x0_center>0 & x0~=0); % right side
        ang0(ind0)=mod(atan(-y0_center(ind0)./x0_center(ind0)),2*pi); %clockwise?
        ind0=find(x0_center<0 & x0~=0);% lift side
        ...
        ang00_unwrap = unwrap(ang00);% remove phase jump?

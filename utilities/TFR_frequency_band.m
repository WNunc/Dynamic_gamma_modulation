...
function [ypower,yphase] = energyvec(f,s,Fs,width)
...
%get power
ypower = (2*abs(y)/Fs).^2;
ypower = ypower(ceil(length(m)/2):length(y)-floor(length(m)/2));

%get phases
yphase =y;
l = find(abs(yphase) == 0);
yphase(l) = 1;
yphase = yphase./abs(yphase);                 
yphase(l) = 0;
yphase = yphase(  ceil(length(m)/2) :  length(yphase)-floor(length(m)/2));
yphase = angle(yphase);

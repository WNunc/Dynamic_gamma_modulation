function [samples, tt, tt_raw, Fs_raw] = loadCSC_new_WN(CSCfile)

%load the csc data
% Original by Sean: [samples, tt_raw, Fs_raw, bv, ir]  = loadEEG(infile);
% edit on 2021年2月4日 by 王宁
% 注释文件名分段 只能在当前文件夹内使用
...
[~,~,~,~,header] = load_eeg_retrieve_header_cz(CSCfile); % CSCfile0
%you'll see the ADBitVolts conversion factor.... e.g., ADBitVolts 0.00644
if isempty(header)
    % read header from CSC file in current dir
    % parameter
    ...
    [~,~,~,~,header] =Nlx2MatCSC(CSCfile,fieldSelection,extractHeader,extractMode);
end
...
disp(['=====   ADBitVolts = ' num2str(ADBitVolts) 'V   =====']);

...
fileoutput1 = 'sgamma_phase_eachcell-3cyc.mat';% sgamma 三个theta周期
        ...
        if ~isempty(thcyc_sl)
            for nth = 1:nthc_sl
                ...
                if indth > size(theta_INFO{nl},1)
                    warning('sg-theta ind 与 all-theta ind 不对应')
                    continue
                end
                
                if nth==1
                    thcyc_onset = theta_INFO{nl}{indth,3}(1);
                    thcyc_offset = theta_INFO{nl}{indth+1,3}(2)-0.005;
                    thseq =  horzcat( theta_INFO{nl}{indth,8}(:,1:end-1),...
                        theta_INFO{nl}{indth+1,8}(:,1:end-1));
                    thseq_cyc = [thcyc_onset, theta_INFO{nl}{indth,3}(2) ,thcyc_offset];
                elseif nth>1 && nth< nthc_sl && (theta_INFO{nl}{indth,1}-theta_INFO{nl}{indth-1,1})~=1
                    thcyc_onset = theta_INFO{nl}{indth,3}(1);
                    thcyc_offset = theta_INFO{nl}{indth+1,3}(2)-0.005;
                    thseq =  horzcat( theta_INFO{nl}{indth,8}(:,1:end-1),...
                        theta_INFO{nl}{indth+1,8}(:,1:end-1));
                    thseq_cyc = [thcyc_onset, theta_INFO{nl}{indth,3}(2) ,thcyc_offset];
                elseif nth>1 && nth< nthc_sl && (theta_INFO{nl}{indth+1,1}-theta_INFO{nl}{indth,1})~=1
                    thcyc_onset = theta_INFO{nl}{indth-1,3}(1);
                    thcyc_offset = theta_INFO{nl}{indth,3}(2)-0.005;
                    thseq =  horzcat( theta_INFO{nl}{indth-1,8}(:,1:end-1),...
                        theta_INFO{nl}{indth,8}(:,1:end-1));
                    thseq_cyc = [thcyc_onset, theta_INFO{nl}{indth-1,3}(2) ,thcyc_offset];
                elseif nth == nthc_sl
                    thcyc_onset = theta_INFO{nl}{indth-1,3}(1);
                    thcyc_offset = theta_INFO{nl}{indth,3}(2)-0.005;
                    thseq =  horzcat( theta_INFO{nl}{indth-1,8}(:,1:end-1),...
                    ...
                    thseq = horzcat(theta_INFO{nl}{indth-1,8}(:,1:end-1),...
                        theta_INFO{nl}{indth,8}(:,1:end-1),...
                        theta_INFO{nl}{indth+1,8}(:,1:end-1));
                    thseq_cyc = [thcyc_onset,theta_INFO{nl}{indth-1,3}(2),...
                        theta_INFO{nl}{indth,3}(2),thcyc_offset];
                end
...
%     % 画图
    plot_cellPhs_in_sgDomThCyc_allLap_3cyc

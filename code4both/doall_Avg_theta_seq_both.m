...
for ns = [1,2,4:6,8,10:13,15,16] % 1:isession
    ...
    nseg = 1;
    m_vel = zeros(3,5); % 平均速度1 = allcycle,2 = goodcycle,3 = ontrack
    sem_vel = zeros(3,5); % 速度标准误
    ...
    for nl = 1:5
        ...
        for D = 1:2
            ...
            %% 画原始解码
            ffdecode = figure('PaperPositionMode','auto');
            if D == 1
%                 plot_decodeingandtheta_CW(scores,thetacycle,nl,TS_running,mth)
%                 saveas(ffdecode,[outFolder, '1-CWphase_', case3, '_decoding_single_lap',num2str(nl),midmod,'.epsc'])
%                 saveas(figure(1),[outFolder, '1-CWphase_', case3, '_decoding_single_lap',num2str(nl),midmod,'.png'])
            else
%                 plot_decodeingandtheta_CCW(scores,thetacycle,nl,TS_running,mth)
%                 saveas(ffdecode,[outFolder, '2-CCWphase_', case3, '_decoding_single_lap',num2str(nl),midmod,'.epsc'])
%                 saveas(figure(1),[outFolder, '2-CCWphase_', case3, '_decoding_single_lap',num2str(nl),midmod,'.png'])
            end
            close 
            ...
            RunSpeed{nl} = [RunSpeed{nl} ;lap_linvel(ind_tsv)];
            ...
            thetaind = find([theta_info{nl}{:,2}]>0.1&[theta_info{nl}{:,2}]<0.2...时间长度在100ms到200ms之间
                ...
                & nonancol>=12 ...非nan的值大于等于12个,60ms
...
%         plot_NPX(NPX2(28:63,:),mid_tbins,cycind,abc)
        ...
        %% 速度统计
        m_vel(1,nl) = mean([theta_INFO{nl}{:,11}],'omitnan');
        sem_vel(1,nl) = std([theta_INFO{nl}{:,11}],'omitnan')./sqrt(size(theta_INFO{nl},1));
        
        m_vel(2,nl) = mean([ThetaGood{:,11}]);
        sem_vel(2,nl) = std([ThetaGood{:,11}])./sqrt(size(ThetaGood,1));
        
        m_vel(3,nl) = mean(RunSpeed{nl});
        sem_vel(3,nl) = std(RunSpeed{nl})./sqrt(size(RunSpeed{nl},1));
        %%
%         save([outFolder,'data_theta_seq_info',case1,'_lap',num2str(nl),case2,case3,midmod,'_v5-both.mat'],...
...
%     ff = figure(6);
%     errorbar([],m_vel',sem_vel','LineWidth',1.5)
%     xlim([0.5 5.5])
%     ylim([5 50])
%     xlabel('Lap Num')
%     ylabel('speed cm/s')
...
%     saveas(figure(2),[outFolder,'thetaseq_strcture',case1,'1',case2,case3,midmod,'-both.png']);clf
%     saveas(figure(3),[outFolder,'thetaseq_strcture',case1,'2',case2,case3,midmod,'-both.png']);clf
%     
%     saveas(figure(4),[outFolder,'thetaseq_pxn',case1,'1',case2,case3,midmod,'-both.png']);clf
%     saveas(figure(5),[outFolder,'thetaseq_pxn',case1,'2',case2,case3,midmod,'-both.png']);clf
%     
%     saveas(ff,[outFolder,'speed_over_lap',case1,case2,case3,midmod,'-both.png']);
%     save([outFolder,'data_theta_seq_info','_AllLap',case1,case2,case3,midmod,'_v5-both.mat'],...

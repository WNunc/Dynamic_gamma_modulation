...
plot_sign = 0;
start_phase=[];PP_feature1=[];PP_feature2=[];
fig_out_dir = 'H:\neuralynx\phase precession rseult\theta phase precession\'
        ...
        for nt = 1% this file for pre_running only
            ...
            for ifgcell = 1:icell
                ...
                PP_feature0 = [ns,...	%1-第几天
                    ...
                    RR];                %9-权重
                PP_feature1 = [PP_feature1;PP_feature0];
                % 画图
                % ==========================画图==================================%
                if plot_sign==1
                    fig = figure;
                    set(fig,'unit','centimeters','position',[25 10 20 12],'Visible','off')
                    hold on
                    ...
                    ylim([0 720]);
                    drawnow
                    output=strcat('FG-cell_Rat',num2str(Rat_id),'_day',num2str(nsn),'_seg',num2str(nt),'_nc',num2str(cellid));
                    set(gcf,'PaperPositionMode','auto') %把figure按照显示的大小保存
                    saveas(gcf,[fig_out_dir,output],'png');
                    saveas(gcf,[fig_out_dir,output],'epsc');
                    close all
                end
            end
            %% NFG PP
            ...
            for ifgcell = 1:icell
                ...
                PP_feature0 = [ns,... %1-第几天
                    ...
                    correlation,...   %8-corr-coeff
                    RR];              %9-权重
                PP_feature2 = [PP_feature2;PP_feature0];
                % 画图
                % ==========================画图==================================%
                if plot_sign==1
                    fig = figure;
                    ...
                    ylim([0 720]);
                    drawnow
                    output=strcat('NFG-cell_Rat',num2str(Rat_id),'_day',num2str(nsn),'_seg',num2str(nt),'_nc',num2str(cellid));
                    set(gcf,'PaperPositionMode','auto') %把figure按照显示的大小保存
                    saveas(gcf,[fig_out_dir,output],'png');
                    saveas(gcf,[fig_out_dir,output],'epsc');
                    close all
...
nFG = size(PP_feature1,1);
nFG_pp = numel(find(PP_feature1(:,5)<0&PP_feature1(:,7)<0.05));
% nFG_pp = numel(find(PP_feature1(:,5)<0));
nNFG = size(PP_feature2,1);
nNFG_pp = numel(find(PP_feature2(:,5)<0&PP_feature2(:,7)<0.05));
% nNFG_pp = numel(find(PP_feature2(:,5)<0));
figure('Position',[752 343 560 238])
subplot(1,2,1)
P1 = pie([nFG_pp,nFG-nFG_pp]);title('FG-cell')
subplot(1,2,2)
P2 = pie([nNFG_pp,nNFG-nNFG_pp]);title('NFG-cell')

figure('Position',[708 151 391 378])
B = bar([(nFG-nFG_pp)/nFG,nFG_pp/nFG;(nNFG-nNFG_pp)/nNFG,nNFG_pp/nNFG],0.8,'stacked');
ylim([0,1.2])

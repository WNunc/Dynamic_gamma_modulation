function plot_decodeingandtheta_CCW(scores,thetacycle,nl,TS_running,mth)
lap_ts = scores{nl,6};%行为学时间
lap_ang = scores{nl,5};%位置（角度）
lap_pxn = flipud(scores{nl,3});%解码位置（Pxn）
% lap_pos = 2*pi-scores{nl,5}(scores{nl,4});%实际位置
lap_pos = 2*pi-scores{nl,9};%实际位置

...
% 匹配两类神经元数量
% Non sg cell
% doall_getnonPhaselock_alllap_bothv3.m
clear
close all
...
for ns = [1,2,4:6,8,10:13,15,16]%1:isession%[1:4,6,7]%[1;2;4;5;6;8;10;11;16;20;22;25]'%[1:11,16,20:22,26,27]%[1;2;5;6;8;10;11;13;20;22;23;24;27;28]'%
    ...
    for D = 1:2
        ...
        load(file_input4)
        load(file_input5)
%         file_input6 = [subfolder1,'scores1-1-ontrack_dssg_TSlap_vel0',lockat{L},'v3.mat'];
%         load(file_input6,'cind_ot_nonsg');
        ...
        for nt = 1% this file for pre_running only
            ...
            if L == 1
                sgamma_rayleighP = [plFeature_singlap{1,2}.rayleighP];% 第一圈算出来的相锁
            elseif L == 2
                sgamma_rayleighP = [plFeature_alllap{1,2}.rayleighP];% 所有圈算出来的相锁
            else
                sgamma_rayleighP = [plFeature_f2lap{1,2}.rayleighP];% 前两圈算出来的相锁
            end
            ...
            cind_ot_sg = find(sgamma_rayleighP_ontrack<0.05 & spkNum_ind'==1) % ontrack的ind % 相锁神经元的ontrack的ind(spkNum_alllap'>50)
            % cind_ot_nonsg = find(sgamma_rayleighP_ontrack>=0.05 & spkNum_ind'==1)
            cind_ot_nonsg = find(sgamma_rayleighP_ontrack>=0.05)
            cellnum_sg(nsn,D) = length(cind_ot_sg);
            cellnum_nonsg(nsn,D) = length(cind_ot_nonsg);
            
            
            %             if cellnum_sg(nsn,1)>cellnum_nonsg(nsn,1) %如果需要排除的非相锁神经元大于非相锁神经元总数，那么直接把所有的非相锁神经元都去掉
            %                 cind_ot_nonsg = cind_ot(~ismember(cind_ot,cind_ot(cind_ot_sg)))
            %                 cellnum_sg(nsn,1) = length(cind_ot_nonsg);
            %                 disp('causion: fast gamma phase locking cell > non phase locking cell')
            %             else
            %                 cind_ot_nonsg = cind_ot(~ismember(cind_ot,cind_dsamp))
            %             end
        end
        ...
        for isgcell = 1:icell
            ...
            saveas(ffp,[outFolder 'NSG ' TT0t64{cellid} '-' num2str(D) '.png']);
            saveas(ffp,[outFolder 'NSG ' TT0t64{cellid} '-' num2str(D) '.eps'],'epsc');
            for nl = 1:5
...
AA = sum(sum(cellnum_sg))
BB = sum(sum(cellnum_nonsg))

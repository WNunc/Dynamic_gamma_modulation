...
for ns = 1:isession%[1;2;5;6;8;10;11;13;20;22;23;24;27;28]'%
    nsn = nsn+1;
    path_ns = path{ns};
    cd(path_ns);
    ...
    for D = 1:2
        ...
        load(file_input4)
        load(file_input5)
        file_input6 = [subfolder1,'scores1-1-ontrack_dsfg_TSlap_vel0',lockat{L},'.mat'];
        load(file_input6,'cind_dsamp');
        ...
        for nt = 1% this file for pre_running only
            ...
            cind_ot_fg = find(fgamma_rayleighP_ontrack<0.05&fgamma_rayleighP_ontrack~=0) % 相锁神经元的ontrack的ind
            cellnum_fg(nsn,1) = length(cind_ot_fg);
            
            if cellnum_fg(nsn,1)>cellnumontrack-cellnum_fg(nsn,1) %如果需要排除的非相锁神经元大于非相锁神经元总数，那么直接把所有的非相锁神经元都去掉

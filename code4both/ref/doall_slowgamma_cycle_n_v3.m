...
% expand cyc-1 and cyc1
% 筛选1.5倍标准差的theta cycle
...
fileinput9 = ['sgamma_dominant_thetacyc_IND_std' num2str(n_std) '_v2.mat'];% sgamma 主导的 theta cycle
fileoutput1 = 'data_CycleList_sg_v3.mat';% sg cycle timestamp
...
for ns = 1:isession
    ...
    ff9dir = [resuletFolder path_ns(13:end)];
    load([ff9dir,fileinput9])
    for nt = 1%1:3
        switch nt
            case 1
                laps=5;
        ...
        for D = 1:2 % CW和CCW两个方向
            ...
            load([ff7dir,fileinput7],'theta_info')
            ListC0{nt,1}=cell(1,laps);
            for nl = 1:laps
                ...
                %%
                tbin = scores{nl,6};% 解码的时间bin
                t_EEG_sample = scores{nl,10};
                info = theta_info{nl};
                % 筛选theta cycle
                if D==1
                    D1(nl)=size(info,1);% 第一圈的theta cycle数
                    ind_sgth= ind_th_sl{nl}( ind_th_sl{nl}<=D1(nl));
                else
                    ind_sgth= ind_th_sl{nl}( ind_th_sl{nl}>D1(nl))-D1(nl);
                end
                ...
                for nc = 1:numberCell
                    ...
                    tt_sg=[];
                    List=[];
                    EEG_sample_sg = scores{nl,17}(CSCd(nc),:);
                    phase_sg=scores{nl,20}(CSCd(nc),:);
                    ind_sg=findpeaks(EEG_sample_sg);
                    ind_sg = ind_sg.loc;
                    ind_sg = ind_sg';
                    yp_sg = EEG_sample_sg(ind_sg);
                    tt_sg=t_EEG_sample(ind_sg);
                    for i=1:L % theta cycle
                        List{i,1}=find(tt_sg > info{i,3}(1) & tt_sg < info{i,3}(2));
                        Peak{i,1}=yp_sg(List{i,1});
                        List{i,2}=length(Peak{i,1});
                        [~,List{i,3}]=max(Peak{i,1});
                        if (List{i,2}-List{i,3})>=1 & List{i,3}>1 & List{i,1}(1,1)~=1%最大峰值cycle在中间的情况，左右都有cycle
                            ListC0{nt,1}{nc,nl}{1,i}=1;
                            List{i,5}=[List{i,1}(1),List{i,1}(List{i,3}),List{i,1}(end)];
                            ind_gc=ind_sg(1,List{i,5});
                            gc1=ind_gc(1);
%                             while EEG_sample_sg(gc1-1)<EEG_sample_sg(gc1)
%                                 gc1=gc1-1;
%                             end
                            gc2=ind_gc(2);
                            while EEG_sample_sg(gc2-1)<EEG_sample_sg(gc2)
                                gc2=gc2-1;
                            end
                            gc3=ind_gc(2);
                            while EEG_sample_sg(gc3+1)<EEG_sample_sg(gc3)
                                gc3=gc3+1;
                            end
                            gc4=ind_gc(3);
%                             while EEG_sample_sg(gc4+1)<EEG_sample_sg(gc4)
%                                 gc4=gc4+1;
%                             end
                        ...
                        elseif List{i,3}==1 & length(List{i,1})~=1 & List{i,1}(1,1)~=1%最大峰值cycle在最开始的情况，只有右边有cycle
                            ListC0{nt,1}{nc,nl}{1,i}=2;
                            List{i,5}=[List{i,1}(List{i,3}),List{i,1}(end)];
                            ind_gc=ind_sg(1,List{i,5});
                            gc1=ind_gc(1);
%                             while EEG_sample_sg(gc1-1)<EEG_sample_sg(gc1)
%                                 gc1=gc1-1;
%                             end
                            ...
                            ListC0{nt,1}{nc,nl}{2,i}=tbin(List{i,8});
                        elseif (List{i,2}-List{i,3})==0 & length(List{i,1})~=1 & i~=L%最大峰值cycle在最末尾的情况，只有左边有cycle
                            ListC0{nt,1}{nc,nl}{1,i}=3;
                            List{i,5}=[List{i,1}(1),List{i,1}(List{i,3})];
                            ind_gc=ind_sg(1,List{i,5});
                            gc1=ind_gc(1);
                            while EEG_sample_sg(gc1-1)<EEG_sample_sg(gc1)
                                gc1=gc1-1;

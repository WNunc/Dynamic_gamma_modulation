...
for ns = 13%1:isession
    ...
    for nl = 1:5
        for D = 1:2
            ...
            if D == 1
                [TSlimit, speed, speed_ang]= TSfind_laplimit_CW(data_angle,Ang_RewardLoc_ontrack,lap_speed,lap_pos,lap_ts,nseg,nl);
            else
                [TSlimit, speed, speed_ang]= TSfind_laplimit_CCW(data_angle,Ang_RewardLoc_ontrack,lap_speed,lap_pos,lap_ts,nseg,nl);
            end
...
%     save([outFolder,'TS_section.mat'],'TS_section')
end
...
ind_ts_1 = max(find(lap_speed'>=10 & lap_pos<=Ang_RewardLoc_ontrack(6)));
ind_ts_2 = max(find(lap_speed'>=10 & lap_pos<=Ang_RewardLoc_ontrack(12)));
...
speed = data_angle{nsegment}{nlap}(:,3);% cm/s
speed_ang = data_angle{nsegment}{nlap}(:,4);% rad/s
ind_ts_1 = max(find(lap_speed'>=10 & lap_pos>=Ang_RewardLoc_ontrack(12)));
ind_ts_2 = max(find(lap_speed'>=10 & lap_pos>=Ang_RewardLoc_ontrack(6)));
TSlimit = [lap_ts(ind_ts_1), lap_ts(ind_ts_2)];
end

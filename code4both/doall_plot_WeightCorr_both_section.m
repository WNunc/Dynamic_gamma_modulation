...
%         xlabel('Lap Num')
        ...
        file_output1 =['Weight_Correlation' ncases Qsize case3 midmod lockat{L} '-part.png'];
    ...
    ff2 = figure;
    for S = 1:3
    WC1_allmean = mean(WC1_all{S},3,'omitnan');
    WC2_allmean = mean(WC2_all{S},3,'omitnan');
    WC1_allsem = std(WC1_all{S},0,3,'omitnan')./sqrt(size(WC1_all{S},3));
    WC2_allsem = std(WC2_all{S},0,3,'omitnan')./sqrt(size(WC2_all{S},3));
    % 画总的平均的WC
    ...
    set(gca,'FontSize',18)
    legend({'allcell','exfgcell','exnonfgcell'},'Position',[0.875,0.84,0.12,0.12])
    saveas(ff2,['H:\neuralynx\gamma in sequence result\Weight Correlation_' lockat{L} '_part.png'])
    save(['H:\neuralynx\gamma in sequence result\Weight Correlation_' lockat{L} '_part.mat'],...
        'WC1_all','WC2_all')
    end
end

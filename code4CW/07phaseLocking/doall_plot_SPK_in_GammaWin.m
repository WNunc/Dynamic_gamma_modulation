    ...
    for nc = 1:Ncell
        ...
        if nc == 1 || (nc>=2 && (~strcmp(Current, CSC0{nc,1})))
            ...
            for iw = 1:length(IND_sg)%slow gamma
                ...
                subplot(2,1,1)
                title(['cw-' TT0{nc} '-SGwin' num2str(iw)])
                saveas(gcf,[outfolder 'cw-' TT0{nc} '-SGwin' num2str(iw) '.png'])
                saveas(gcf,[outfolder 'cw-' TT0{nc} '-SGwin' num2str(iw) '.eps'],'epsc')
            end
            for iw = 1:length(IND_fg)%fast gamma
                ...
                subplot(2,1,1)
                title(['cw-' TT0{nc} '-FGwin' num2str(iw)])
                saveas(gcf,[outfolder 'cw-' TT0{nc} '-FGwin' num2str(iw) '.png'])
                saveas(gcf,[outfolder 'cw-' TT0{nc} '-FGwin' num2str(iw) '.eps'],'epsc')
            end
            close all
        else
